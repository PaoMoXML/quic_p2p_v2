...
mod tests {
    ...
    fn join_and_leave_works() {
        let mut nodes = vec![
            Node::new("foo", rand::rng()),
            Node::new("bar", rand::rng()),
            Node::new("baz", rand::rng()),
        ];
        ...
        // re-join
        nodes.push(Node::new("baz", rand::rng()));
        nodes[2].join("bar");
    ...
    fn limit_active_view_size() {
        ...
        let mut nodes = vec![
            Node::with_options("foo", rand::rng(), options.clone()),
            Node::with_options("bar", rand::rng(), options.clone()),
            Node::with_options("baz", rand::rng(), options.clone()),
            Node::with_options("qux", rand::rng(), options.clone()),
        ];
